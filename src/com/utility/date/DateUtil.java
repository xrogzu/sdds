...
public class DateUtil {
    ...
    public static SimpleDateFormat sf2 = new SimpleDateFormat("yyyy");
    public static SimpleDateFormat sf3 = new SimpleDateFormat("yyMM");
    public static SimpleDateFormat sf4 = new SimpleDateFormat("yyMMdd");
    public static SimpleDateFormat sf4_2 = new SimpleDateFormat("yyyyMM");
    public static SimpleDateFormat sf5 = new SimpleDateFormat("yyyy.MM");
    public static SimpleDateFormat sf6 = new SimpleDateFormat("yyyy.MM.dd");
    public static SimpleDateFormat sf7 = new SimpleDateFormat("yy.MM");
    public static SimpleDateFormat sf8 = new SimpleDateFormat("yy.MM.dd");
    public static SimpleDateFormat sf11 = new SimpleDateFormat("yyyy-MM");
    public static SimpleDateFormat sf12 = new SimpleDateFormat("yyyy-MM-dd");
    public static SimpleDateFormat sf13 = new SimpleDateFormat("yy-MM");
    public static SimpleDateFormat sf14 = new SimpleDateFormat("yy-MM-dd");
    public static SimpleDateFormat sf15 = new SimpleDateFormat("yyyy/MM");
    public static SimpleDateFormat sf16 = new SimpleDateFormat("yyyy/MM/dd");
    public static SimpleDateFormat sf17 = new SimpleDateFormat("yy/MM");
    public static SimpleDateFormat sf18 = new SimpleDateFormat("yy/MM/dd");
    public static SimpleDateFormat sf19 = new SimpleDateFormat("dd/MM/yyyy");
	
	/**
	 * support:19990101,1999,9901,990101,
	 * 			1999.01,1999.1,1999.01.01,1999.1.1,99.1,99.1.1
	 * 			1999-01,1999-1,1999-01-01,1999-1-1,99-1,99-1-1
	 * 			1999/01,1999/1,1999/01/01,1999/1/1,99/1,99/1/1
	 * @param dateStr
	 * @return
	 */
	private static Date handleDate(String dateStr) {
		Date d = null;
		if (dateStr == null || dateStr.trim().length() < 4) {
//			System.out.println(dateStr + ";;;无法识别的日期格式");
//			Calendar c = Calendar.getInstance();
//			c.set(Calendar.YEAR, 1900);
//			return c.getTime();
			return d;
		}
		dateStr = dateStr.trim().replaceAll("　", "").replaceAll("‘", "").replaceAll("年", "-").replaceAll("月", "-").replaceAll("，", "").replaceAll("正式党员", "");
		try {
			if (dateStr.contains(".")) {
				if (dateStr.split("\\.").length >= 3) {
					String str1 = dateStr.split("\\.")[0].trim();
					if (str1.length() == 4) {
						d = sf6.parse(dateStr);
					} else {
						d = sf8.parse(dateStr); 
...
//					System.out.println(dateStr + ";;;无法识别的日期格式");
				...
				if (dateStr.split("-").length >= 3) {
					String str1 = dateStr.split("-")[0].trim();
					if (str1.length() == 4) {
						d = sf12.parse(dateStr);
					} else {
						d = sf14.parse(dateStr); 
					}
				} else if (dateStr.split("-").length >= 2) {
					String str1 = dateStr.split("-")[0].trim();
					if (str1.length() == 4) {
						d = sf11.parse(dateStr);
					} else {
						d = sf13.parse(dateStr); 
					}
				} else {
//					System.out.println(dateStr + ";;;无法识别的日期格式");
					return d;
				...
				if (dateStr.split("/").length >= 3) {
					String str1 = dateStr.split("/")[0].trim();
					String str3 = dateStr.split("/")[2].trim();
					if (str1.length() == 4) {
						d = sf16.parse(dateStr);
					} else if (str3.length()==4){
						d = sf19.parse(dateStr); 
					} else {
						d = sf18.parse(dateStr); 
					}
				} else if (dateStr.split("/").length >= 2) {
					String str1 = dateStr.split("/")[0].trim();
					if (str1.length() == 4) {
						d = sf15.parse(dateStr);
					} else {
						d = sf17.parse(dateStr); 
					}
				} else {
//					System.out.println(dateStr + ";;;无法识别的日期格式");
					return d;
				...
				if (dateStr.length() == 8) {
					...
					int two = Integer.parseInt(dateStr.substring(0, 2));
					if (two > 20) {
						d = sf4.parse(dateStr);
					} else {
						d = sf4_2.parse(dateStr);
					}
