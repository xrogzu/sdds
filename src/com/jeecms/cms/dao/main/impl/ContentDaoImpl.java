...
public class ContentDaoImpl extends HibernateBaseDao<Content, Integer>
	...
	public Pagination getPage(String title, Integer typeId,Integer currUserId,
		...
		Finder f = Finder.create("select distinct bean from Content bean left join bean.contentShareCheckSet shareCheck left join shareCheck.channel tarChannel ");
		if (rejected == status) {
	...
	public Pagination getPageByRight(String title, Integer typeId,Integer currUserId,
		...
		if (channelId != null) {
			...
			f.append(",Channel parent");//f.append(" where ((channel.lft between parent.lft and parent.rgt");
			f.append(" where ((channel.lft between parent.lft and parent.rgt");
			f.append(" and channel.site.id=parent.site.id");
			f.append(" and parent.id=:parentId");
			if(departId!=null){
			...
			f.append(" or (shareCheck.checkStatus<>0 and shareCheck.shareValid=true and  tarChannel.lft >= parent.lft and tarChannel.rgt <=parent.rgt and tarChannel.site.id=parent.site.id and parent.id=:parentId)))" );
			f.setParam("parentId", channelId);
	...
	private Finder byChannelPaths(String[] paths, Integer[] siteIds,
		...
		Finder f = Finder.create();
		f.append("select distinct bean from Content bean join bean.channel channel ");
		f.append(" left join bean.contentShareCheckSet shareCheck left join shareCheck.channel tarChannel ");
		appendJoinConentDoc(f, open);
		f.append(" join bean.contentExt as ext");
	...
	*/
	...
	@Override
	public Content updateContentInfo(Content content) {
