...
public class FrameAct {
	...
	public String userLeft(ModelMap model,HttpServletRequest request) {
		CmsUser user = CmsUtils.getUser(request);
		CmsDepartment dept = user.getDepartment();
		String ssxz = "";//区别省市县支部
		if(dept.getId()==1){
			ssxz = "sheng";
		}else if(!"支部".equals(dept.getSddspoOrgtype()) && (dept.getParent().getParent()==null)){
			ssxz = "shi";
		}else if(!"支部".equals(dept.getSddspoOrgtype()) && (dept.getParent().getParent()!=null)){
			ssxz = "xian";
		}else{
	...
	public String statisticLeft(ModelMap model,HttpServletRequest request){
		CmsUser user = CmsUtils.getUser(request);
	...
	/**
	 * 规范化党支部
	 * @normalizeorgMain() 主框架
	 * @normalizeorgLeft() 左框体
	 * @normalizeorgRight() 右框体
	 */
	@RequestMapping("/frame/normalizeorg_main.do")
	public String normalizeorgMain(){
		return "frame/normalizeorg_main";
	}
	
	@RequestMapping("/frame/normalizeorg_left.do")
	public String normalizeorgLeft(HttpServletRequest request,
		...
		model.addAttribute("user", user.getUsername());
		return "frame/normalizeorg_left";
	}
	
	@RequestMapping("/frame/normalizeorg_right.do")
	public String normalizeorgRight(){
		return "frame/normalizeorg_right";
	}
	/**
	 * 党费计算器 
	 * @param bean
	 ...
	 * @return
	 */
	@RequestMapping("/duesCalculator/duesCalculator.do")
	public String duesCalculator(){
		return "duescalculator/duesCalculator";
	}
	/**
	 * 发展党员流程展示
	 */
	@RequestMapping("/development_partymembers/development_main.do")
	 ...
	 * 发展党员流程管理
	 */
	@RequestMapping("/development_partymembers/development_manage.do")
	public String adddevelop(){
		return "development_partymembers/development_manage";
	}
	@RequestMapping("/scoreuserxf/scoreUserXFmain.do")
	public String toScoreUserXFmain(){
		return "scoreuserxf/scoreUserXFmain";
	}
