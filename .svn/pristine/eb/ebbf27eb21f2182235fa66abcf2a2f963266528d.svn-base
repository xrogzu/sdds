...
public class ContentShareCheckDaoImpl extends
	...
	@SuppressWarnings("unchecked")
	public List<ContentShareCheck> findByContentId(Integer contentId) {
		Finder finder = Finder.create("from ContentShareCheck bean where bean.content.id=:contentId");
		finder.setParam("contentId", contentId);
		return find(finder);
	}
	
	public ContentShareCheck findByChannelIdAndContentId(Integer channelId, Integer contentId) {
		Finder f = Finder.create("from ContentShareCheck bean where bean.channel.id=:channelId and bean.content.id=:contentId");
		f.setParam("channelId", channelId).setParam("contentId", contentId);
		Query query = f.createQuery(getSession());
		return (ContentShareCheck) query.uniqueResult();
	}
