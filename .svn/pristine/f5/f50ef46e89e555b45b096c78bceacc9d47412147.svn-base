...
public class WelcomeAct {
	...
	public String top(HttpServletRequest request, ModelMap model) {
		...
		//channelMng.findByIdAndDepartId(id, departId)(depart.getId());
		String channel="";
		if(depart.getId()!=1){
			if(depart.getSddspoOrgtype().equals("机关党委") || depart.getSddspoOrgtype().equals("党总支")){
				channel=channelMng.getChannelPathByDid(depart.getParent().getId());
			}else{
				channel=channelMng.getChannelPathByDid(depart.getId());
			}
		}else{
			channel=channelMng.getChannelPathByDid(depart.getId());
		}
		//String names = channel.getChannelExt().getChannel().get;
		//System.out.println("channel:"+cha;
        //System.out.println("sitePath"+site.getAccessPath());
		model.addAttribute("siteList", siteList);
		//System.out.println("栏目路径:"+user.getDepartment().getChannelPath());
		model.addAttribute("channel",channel);
		model.addAttribute("site", site);
		//System.out.println("requestPath:"+request.getServerName());
		
		model.addAttribute("sitePath",site.getAccessPath());
		model.addAttribute("siteParam", AdminContextInterceptor.SITE_PARAM);
		model.addAttribute("user", user);
		model.addAttribute("requestPath", request.getServerName());
		model.addAttribute("deptType",depart.getSddspoOrgtype());
		model.addAttribute("deptLevel", depart.getSddspoOrglevel());
		return "top";
	}
	...
	private void getSitelStatic(HttpServletRequest request, ModelMap model){
